...
prepare_data <- function(input, nscm_data){
...
get_nosocomial_plots_by_case_id <- function(input, nscm_data){
  nscm_data <- prepare_data(input, nscm_data)
  #true date of onset  
  nscm_data$reported_onset_date <- ifelse(nscm_data$death_avail==1,(as.Date(nscm_data$DateDeath, format="%d/%m/%Y") - nscm_data$days_onset_to_death),
  ...
  pat_melted <- data.table::melt(setDT(pat_gantt), c("CaseID","infectionNosocomial","earlyOnset", "reported_onset_date"), measure=list(colsA, colsB, colsC), value.name=c("Hospital","Start_Date","End_Date"))
  ...
  tooltip <- paste0(pat_melted$Hospitalization_Dates, " to ", pat_melted$End_Date, ". Date Onset: ", pat_melted$reported_onset_date)
...
get_nosocomial_plots_by_hospital_patients <- function(input, nscm_data){
  ...
  pat_gantt <- as.data.frame(nscm_data %>% group_by(Surname,OtherNames) %>% select(id, HospitalCurrent,DateHospitalCurrentAdmit, DateDeath, HospitalBeforeIllName,HospitalBeforeIllDateStart, HospitalBeforeIllDateEnd, HospitalPast1, DateHospitalPastStart1, DateHospitalPastEnd1, HospitalPast2, DateHospitalPastStart2, DateHospitalPastEnd2, reported_onset_date, infectionNosocomial, Surname, OtherNames)) %>% arrange(Surname)
  pat_gantt$CaseID <- paste0(pat_gantt$id)
  pat_gantt$reported_onset_date <- pat_gantt$reported_onset_date
  pat_gantt$earlyOnset <-  (as.Date(pat_gantt$DateHospitalCurrentAdmit) < pat_gantt$reported_onset_date) #(as.Date(hosp_gantt$DateOnset, format="%d/%m/%Y") >= as.Date(hosp_gantt$Admission_date)-hosp_gantt$incubation_period)    
  pat_gantt$infectionNosocomial <- ifelse(pat_gantt$earlyOnset == TRUE, "Nosocomial", "Not Nosocomial")
...
get_nosocomial_plots_by_hospital <- function(input, nscm_data){
  ...
  hosp_gantt <- as.data.frame(nscm_data %>% group_by(HospitalCurrent) %>% select(HospitalCurrent,DateHospitalCurrentAdmit, EndDate, reported_onset_date, infectionNosocomial) %>% arrange(HospitalCurrent))
  names(hosp_gantt) <- c('Hospital','Start_date','End_date','reported_onset_date','Nosocomial_status')
  ...
  hosp_gantt$earlyOnset <-  (as.Date(hosp_gantt$Start_date) < as.Date(hosp_gantt$reported_onset_date, format="%d/%m/%Y")) #(as.Date(hosp_gantt$DateOnset, format="%d/%m/%Y") >= as.Date(hosp_gantt$Admission_date)-hosp_gantt$incubation_period)    
  ...
  tooltip <- paste0(hosp_gantt$Hospitalization_Dates, " to ", hosp_gantt$End_date, ". Date Onset: ", hosp_gantt$reported_onset_date)
